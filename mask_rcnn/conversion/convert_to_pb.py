...
def main(args=None):
    ...
    output_nodes = ['mrcnn_detection/Reshape_1', "mrcnn_mask/Reshape_1"]
    convert_model(model_A, output_nodes)


def convert_model(inference_model, output_nodes=[]):
    # convert the keras model to pb
    #orig_output_node_names = [node.op.name for node in inference_model.outputs]
    orig_output_node_names = output_nodes
