...
def replace_yolopb_h_and_w(h,w):
    """Replace yolo.pbtxt High and Wide"""
    pb_path = "./models_warmup/yolo/yolo.pbtxt"
    print("replace_yolopb_h_and_w:{},{},{}".format(pb_path,h,w))
    with open(pb_path, "rt") as file:
        data = file.read()
    with open(pb_path, "wt") as file:
...
class DarkNetParser(object):
    ...
    def _parse_params(self, param_line, skip_params=None):
        ...
        elif param_type == 'mask' or param_type == 'anchors':
...
def main():
    ...
    output_file_path = os.path.join(model_dir, 'model.onnx')
    onnx.save(yolo_model_def, output_file_path)
    #subprocess.run(["bash", "/opt/tensorrtserver/build_config.sh", model_name])
    print('Done.')
