...
class FeatureBRSPredictor(BRSBasePredictor):
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        def get_prediction_logits(scale, bias):
            ...
            if self.insertion_mode == 'after_c4':
                ...
                #print("feature_extractor.head input1:" + str(scaled_backbone_features.size()))
                [scaled_backbone_features] = detect([scaled_backbone_features.detach().cpu().numpy()], "feature_extractor.head")
                #scaled_backbone_features = self.net.feature_extractor.head(scaled_backbone_features)
                #print("feature_extractor.head output1:" + str(scaled_backbone_features.size()))
            
            #print("head input1:" + str(scaled_backbone_features.size()))
            [pred_logits] = detect([scaled_backbone_features], "head")
            #pred_logits = self.net.head(scaled_backbone_features)
            #print("head output1:" + str(pred_logits.size()))
            pred_logits = torch.from_numpy(pred_logits).to(self.device)
    ...
    def _get_head_input(self, image_nd, points):
        with torch.no_grad():
            #print("dist_maps input1:" + str(image_nd.size()))
            #print("dist_maps input2:" + str(points.size()))
            coord_features = self.net.dist_maps(image_nd, points)
            #print(image_nd.type())
            #print(points.type())
            #coord_features = detect([image_nd.numpy().astype(np.float32), points.numpy().astype(np.int64)], "dist_maps")
            #print("dist_maps output1:" + str(coord_features.size()))
            #print("rgb_conv input1:" + str(torch.cat((image_nd, coord_features), dim=1).size()))
            #x = self.net.rgb_conv(torch.cat((image_nd, coord_features), dim=1))
            [x] = detect([torch.cat((image_nd, coord_features), dim=1).cpu().numpy()], "rgb_conv")
            #print("rgb_conv output1:" + str(x.size()))
            if self.insertion_mode == 'after_c4' or self.insertion_mode == 'after_aspp':
                #print("feature_extractor.backbone input1:" + str(x.size()))
                #c1, c2, c3, c4 = self.net.feature_extractor.backbone(x)
                c1, c2, c3, c4 = detect([x], "feature_extractor.backbone")
                #print("feature_extractor.backbone output1:" + str(c1.size()))
                #print("feature_extractor.backbone output2:" + str(c2.size()))
                #print("feature_extractor.backbone output3:" + str(c3.size()))
                #print("feature_extractor.backbone output4:" + str(c4.size()))
                #print("feature_extractor.skip_project input1:" + str(c1.size()))
                #c1 = self.net.feature_extractor.skip_project(c1)
                [c1] = detect([c1], "feature_extractor.skip_project")
                #print("feature_extractor.skip_project output1:" + str(c1.size()))

                if self.insertion_mode == 'after_aspp':
                    #print("feature_extractor.aspp intput1:" + str(c4.size()))
                    #x = self.net.feature_extractor.aspp(c4)
                    [x] = detect([c4], "feature_extractor.aspp")
                    x = torch.from_numpy(x).to(self.device)
                    c1 = torch.from_numpy(c1).to(self.device)
                    #print("feature_extractor.aspp output1:" + str(x.size()))
...
class HRNetFeatureBRSPredictor(BRSBasePredictor):
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        def get_prediction_logits(scale, bias):
            ...
            scaled_backbone_features = scaled_backbone_features + bias
            if self.insertion_mode == 'A':
                #print("feature_extractor.aux_head input1:" + str(scaled_backbone_features.size()))
                [out_aux] = detect([scaled_backbone_features.detach().cpu().numpy()], "feature_extractor.aux_head")
                #out_aux = self.net.feature_extractor.aux_head(scaled_backbone_features)
                #print("feature_extractor.aux_head output1:" + str(out_aux.size()))
                #print("feature_extractor.conv3x3_ocr input1:" + str(scaled_backbone_features.size()))
                [feats] = detect([scaled_backbone_features.detach().cpu().numpy()], "feature_extractor.conv3x3_ocr")
                #feats = self.net.feature_extractor.conv3x3_ocr(scaled_backbone_features)
                #print("feature_extractor.conv3x3_ocr output1:" + str(feats.size()))

                #print("feature_extractor.ocr_gather_head input1:" + str(feats.size()))
                #print("feature_extractor.ocr_gather_head input2:" + str(out_aux.size()))
                [context] = detect([feats, out_aux], "feature_extractor.ocr_gather_head")
                #context = self.net.feature_extractor.ocr_gather_head(feats, out_aux)
                #print("feature_extractor.ocr_gather_head output1:" + str(context.size()))
                #print("feature_extractor.ocr_distri_head input1:" + str(feats.size()))
                #print("feature_extractor.ocr_distri_head input2:" + str(context.size()))
                [feats] = detect([feats, context], "feature_extractor.ocr_distri_head")
                #feats = self.net.feature_extractor.ocr_distri_head(feats, context)
                #print("feature_extractor.ocr_distri_head output1:" + str(feats.size()))
                #print("feature_extractor.cls_head input1:" + str(feats.size()))
                [pred_logits] = detect([feats], "feature_extractor.cls_head")
                #pred_logits = self.net.feature_extractor.cls_head(feats)
                #print("feature_extractor.cls_head output1:" + str(pred_logits.size()))
            elif self.insertion_mode == 'C':
    ...
    def _get_head_input(self, image_nd, points):
        with torch.no_grad():
            ...
            [x] = detect([torch.cat((image_nd, coord_features), dim=1).cpu().numpy()], "rgb_conv")
            #x = self.net.rgb_conv(torch.cat((image_nd, coord_features), dim=1))
            #print("feature_extractor.compute_hrnet_feats input1:" + str(x.shape))
            [feats] = detect([x], "feature_extractor.compute_hrnet_feats")
            #feats = self.net.feature_extractor.compute_hrnet_feats(torch.from_numpy(x))
            #feats = self.net.feature_extractor.compute_hrnet_feats(x)
            #print("feature_extractor.compute_hrnet_feats output1:" + str(feats.size()))
