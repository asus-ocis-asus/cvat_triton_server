...
from .initializer import Bilinear
...
class BilinearConvTranspose2d(nn.ConvTranspose2d):
    def __init__(self, in_channels, out_channels, scale, groups=1):
        ...
        self.apply(Bilinear(scale=scale, in_channels=in_channels, groups=groups))
    ...
    def get_coord_features(self, points, batchsize, rows, cols):
        if self.cpu_mode:
            ...
            for i in range(batchsize):
                ...
                coords.append(get_dist_maps(points[i].cpu().numpy(), rows, cols,
                                            norm_delimeter))
