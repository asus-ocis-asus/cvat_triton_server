...
class ResDown(MultiStageFeature):
    ...
    def forward(self, x):
        output = detect([x], "feature_extractor")
        #output = self.features(x)
        p3 = detect([output[-1]], "feature_downsampler")[0]
...
class Custom(nn.Module):
    ...
    def track_mask(self, search):
        ...
        rpn_pred_cls, rpn_pred_loc = detect([self.zf, self.search], "rpn_model")
        ...
        pred_mask = detect([self.corr_feature], "mask_head")[0]
        #pred_mask = self.mask_model.mask.head(self.corr_feature)
        return rpn_pred_cls, rpn_pred_loc, pred_mask

    def track_refine(self, pos):

        pred_mask = detect([self.feature[0], self.feature[1], self.feature[2], self.corr_feature, np.array(pos)], "refine_model")[0]
