...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode=''):
...
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    ...
    if mask_enable:
        score, delta, mask = net.track_mask(x_crop.numpy())
    else:
        score, delta = net.track(x_crop)

    delta = to_torch(delta)
    score = to_torch(score)
    #delta = np.reshape(np.transpose(delta, (1, 2, 3, 0)), (4, -1)).astype(np.float64)
    ...
    if mask_enable:
        ...
        if refine_enable:
            mask = to_torch(net.track_refine((delta_y, delta_x))).sigmoid().squeeze().view(
