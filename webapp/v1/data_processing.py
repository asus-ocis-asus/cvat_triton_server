...
LABEL_FILE_PATH = os.environ["LABEL_FILE_PATH"]
...
class PreprocessYOLO(object):
    ...
    def process(self, input_image_b64):
        ...
        """
        image_raw, image_resized = self._load_and_resize(input_image_b64)
        image_preprocessed = self._shuffle_and_normalize(image_resized)
        return image_raw, image_preprocessed

    def _load_and_resize(self, input_image_b64):
        ...
        image_data = re.sub('^data:image/.+;base64,', '', input_image_b64)
        #print(image_data)
        image_raw = Image.open(BytesIO(base64.b64decode(image_data))).convert('RGB')
